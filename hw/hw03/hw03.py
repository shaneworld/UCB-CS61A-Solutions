...
def num_eights(n):
    ...
    if n == 0:
        return 0
    return 1 + num_eights(n // 10) if n % 10 == 8 else 0 + num_eights(n // 10)
...
def interleaved_sum(n, f_odd, f_even):
    ...
    def helper(x, fn):
        if x > n:
            return 0
        return fn(x) + helper(x+1, f_even if fn == f_odd else f_odd)
...
def count_dollars_upward(total):
    ...
    def helper(amount, bill):
        if amount == 0:
            ...
            return helper(amount, next_larger_dollar(bill)) + helper(amount - bill, bill)
...
def knapsack(weights, values, c):
    ...
    def helper(i, capacity):
        if capacity == 0 or i == 0:
            return 0
        best = helper(i-1, capacity)
        if weights[i-1] <= capacity:
            best = max(best, values[i-1] + helper(i-1, capacity - weights[i-1]))
        return best
    return helper(len(weights), c)
