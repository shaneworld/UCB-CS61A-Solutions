...
def about(keywords: list[str]):
    ...
    def helper(paragraph):
        rp = remove_punctuation(paragraph)
        sp = split(rp)
        for word in sp:
...
def accuracy(typed: str, source: str) -> float:
    ...
    lt = len(typed_words)
    ls = len(source_words)
    if lt == 0 and ls == 0: return 100.0
    if lt == 0 or ls == 0: return 0.0
...
def wpm(typed: str, elapsed: int) -> float:
    ...
    # BEGIN PROBLEM 4
    length = len(typed)
    return round(length / 5 * 60 / elapsed, 2)
    # END PROBLEM 4
...
def autocorrect(typed_word: str, word_list: list[str], diff_function, limit: int) -> str:
    ...
    rs = min(word_list, key=lambda x: diff_function(typed_word, x, limit))
    diff = diff_function(typed_word, rs, limit)
    return rs if diff <= limit else typed_word
    # END PROBLEM 5
...
def furry_fixes(typed: str, source: str, limit: int) -> int:
    ...
    if limit < 0:
        return 0
    return 1 + furry_fixes(typed[1:], source[1:], limit-1) if typed[0] != source[0] else furry_fixes(typed[1:], source[1:], limit)
    # END PROBLEM 6
...
def report_progress(typed: list[str], source: list[str], user_id: int, upload) -> float:
    ...
    rs = {"id": user_id, "progress": progress}
    upload(rs)
    return progress
    # END PROBLEM 8
...
def time_per_word(words: list[str], timestamps_per_player: list[list[int]]) -> dict:
    ...
    # BEGIN PROBLEM 9
    times = [[b - a for a, b in zip(row, row[1:])] for row in tpp]
    # END PROBLEM 9
...
def fastest_words(words_and_times: dict) -> list[list[str]]:
    ...
    # BEGIN PROBLEM 10
    words, times = words_and_times['words'], words_and_times['times']
    rs = [[] for _ in range(len(times))]
    for j, word in enumerate(words):
        i = min(range(len(times)), key=lambda k: get_time(times, k, j))
        rs[i].append(word)
    return rs
    # END PROBLEM 10
