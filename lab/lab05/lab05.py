...
def sprout_leaves(t, leaves):
    ...
    """
    if is_leaf(t):
        new_tree = tree(label(t), [tree(k) for k in leaves])
        return new_tree
    else:
...
def pathsum(t, n):
    ...
    """
    if is_leaf(t):
        return True if n == label(t) else False
    else:
        rs = [pathsum(b, n-label(t)) for b in branches(t)]
        return True if any(rs) else False
...
def balanced(t):
    ...
    if is_leaf(t):
        ...
        rs = [sum_tree(b) for b in branches(t)]
        return all(x == rs[0] for x in rs) and all(balanced(b) for b in branches(t))
